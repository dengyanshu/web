...
<%@ page language="java" import="com.data.connectsql.TjDatabase"%>
...
<meta http-equiv="refresh" content="600">
	...
	ResultSet rs = null;
	List data = null;
	StringBuffer result=new StringBuffer("<chart caption='工单:"+mo+"' subCaption='各站别每小时产出状况' numdivlines='15' lineThickness='2' showValues='0' anchorRadius='3' anchorBgAlpha='50' showAlternateVGridColor='1' numVisiblePlot='12' animation='0'>");
	try {
		// 存储过程 TEST_MICHAEL_NOOUT 其实是向数据库插入一条数据  
		conn = new TjDatabase().getConn();
		...
		for(int i=0;i<colNameArr.length;i++){
			if(i==0){
				for(int j=0;j<sum;j++){
					...
					result.append("<category label='"+title.get(colNameArr[0])+"' />");
				}
				result.append("</categories>");
			}else{
...
<table width="98%" border="0" cellspacing="0" cellpadding="30" align="center" >
  <tr> 
    <td valign="top" class="text" align="center"> <div id="chartdiv" align="center"> 
      ...
      <%
      	if(data.size()<1){
    		out.print("Ext.onReady(function(){Ext.Msg.alert(\"系统提示\",\"没有查找到数据!\");})");
      	}else{
      %>
	   		var chart = new FusionCharts("js/ScrollLine2D.swf", "CharasdftId", "900", "600", "0", "0");
