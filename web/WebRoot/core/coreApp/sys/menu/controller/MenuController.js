		...
		this.control({
			...
			"panel[xtype=menu.menutree] button[ref=treeIns]":{
				click:function(btn){
					...
					var resObj=self.ajax({url:"/web/pc/menuAction!doSave.action",params:params});
					if(resObj.success){
			...
			"panel[xtype=menu.menutree] button[ref=treeDel]":{
				click:function(btn){
					...
					var node=records[0];
					Ext.Msg.confirm("系统提示","你真的想删除"+records[0].data.text+"?",function(btn){
						if(btn=="yes"){
							var resObj=self.ajax({url:"/web/pc/menuAction!doRemove.action",params:{ids:node.get("id")}});
							if(resObj.success){
								tree.getStore().load();
			...
			"panel[xtype=menu.menuform] button[ref=submit]":{
				click:function(btn){
					...
					if(idValue!=null && idValue!=""){
						formObj.submit({
							url:"/web/pc/menuAction!doUpdate.action",
