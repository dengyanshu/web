...
Ext.define("core.kanban_test.controller.Controller", {
		...
		this.control({
			...
			'panel[xtype=tj_sc_zm_kb]':{	
				itemclick : function(e, eOpts) {
					...
					var win=Ext.create("core.kanban_test.view.item.ZmSc_List",{
						tbar:[
					...
					var store=Ext.data.StoreManager.map['core.kanban_test.store.zm.ListStore'];
					tj_sc_zm_list_task={
						run:function(){
							mask.show();
							store.removeAll();
			...
			'panel[xtype=tj_sc_led_kb]':{	
				itemclick : function(e, eOpts) {
					...
					var WorkcenterName=eOpts.data.WorkcenterName;
					var Workcenterid=eOpts.data.Workcenterid;
					var win=Ext.create("core.kanban_test.view.item.LedSc_List",{
						tbar:[
							{xtype:'tbspacer',width:320},
							'产品名称:('+productDescription+')',
							'-',
					...
					var mask=self.msg(win);
					var store=Ext.data.StoreManager.map['core.kanban_test.store.led.ListStore'];
					tj_sc_led_list_task={
						run:function(){
							mask.show();
							store.removeAll();
	...
	views : [
				'core.kanban_test.base.BaseTree',
				'core.kanban_test.view.MainLayout',
				'core.kanban_test.view.ItemTree',
				'core.kanban_test.view.DisplayPanel',
				'core.kanban_test.view.item.ZmSc',
				'core.kanban_test.view.item.ZmSc_List',
				'core.kanban_test.view.item.LedSc',
				'core.kanban_test.view.item.LedSc_List'
			],

	stores : [
			'core.kanban_test.store.Tree',
			'core.kanban_test.store.zm.Store',
			'core.kanban_test.store.zm.ListStore',
			'core.kanban_test.store.led.Store',
			'core.kanban_test.store.led.ListStore'
			],
			
	models:[
		'core.kanban_test.model.zm.Model',
		'core.kanban_test.model.zm.ListModel',
		'core.kanban_test.model.led.Model',
		'core.kanban_test.model.led.ListModel'
	]
