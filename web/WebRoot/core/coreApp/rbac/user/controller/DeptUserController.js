		...
		this.control({
			...
			"panel[xtype=rbac.depttree] button[ref=treeIns]":{
				click:function(btn){
					...
					var resObj=self.ajax({url:"/web/rbac/deptAction!doSave.action",params:params});
					if(resObj.success){
			...
			"panel[xtype=rbac.depttree] button[ref=treeDel]":{
				click:function(btn){
					...
					var node=records[0];
					var resObj=self.ajax({url:"/web/rbac/deptAction!doRemove.action",params:{ids:node.get("id")}});
					if(resObj.success){
						tree.getStore().load();
			...
			"panel[xtype=rbac.deptform] button[ref=submit]":{
				click:function(btn){
					...
					if(params.deptId!=null && params.deptId!=""){
						var resObj=self.ajax({url:"/web/rbac/deptAction!doUpdate.action",params:params});
