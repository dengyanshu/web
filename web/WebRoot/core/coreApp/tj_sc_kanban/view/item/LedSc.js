Ext.define("core.tj_sc_kanban.view.item.LedSc", {
		...
		,
		{
		    ...
		    handler:function(){
		    	...
		    	if(null != Ext.getCmp("bancisjled").getValue())
		    		{
		    	var chaxunrq= Ext.getCmp("chaxunrqled").getValue();
//		    	alert('查询日期:'+Ext.Date.format(new Date(chaxunrq),'Y-m-d'));
		    	var bancisj=Ext.getCmp("bancisjled").getValue();
//		    	alert('查询时间:'+bancisj);
		    	 var rq_time=Ext.Date.format(new Date(chaxunrq),'Y-m-d')+' '+bancisj;
		    	//alert('查询:'+rq_time);
		    	var store=Ext.data.StoreManager.map['core.tj_sc_kanban.store.led.Store'];
		    	Ext.Ajax.request({
		    		url :'/web/kanban/tj_sc_led!getResult.action' ,
		    		params:{limit:17,page:1,LedTime:rq_time},
