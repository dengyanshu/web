...
Ext.define("core.tj_sc_kanban.view.item.SmtSc", {
			extend : "Ext.grid.Panel",
			alias : 'widget.tj_sc_smt_kb',
			loadMask : true,
			stripeRows : true,
			store : 'core.tj_sc_kanban.store.smt.Store',
			...
			,
			{
			    ...
			    handler:function(){
			    	...
			    	if(null != Ext.getCmp("bancisjsmt").getValue())
			    		{
			    	var chaxunrq= Ext.getCmp("chaxunrqsmt").getValue();
			    	//alert('查询日期:'+Ext.Date.format(new Date(chaxunrq),'Y-m-d'));
			    	var bancisj=Ext.getCmp("bancisjsmt").getValue();
			    	//alert('查询时间:'+bancisj);
			    	 var rq_time=Ext.Date.format(new Date(chaxunrq),'Y-m-d')+' '+bancisj;
			    	//alert('查询:'+rq_time);
			    	var store=Ext.data.StoreManager.map['core.tj_sc_kanban.store.smt.Store'];
			    	Ext.Ajax.request({
			    		url :'/web/kanban/tj_sc_smt!getResult.action' ,
			    		params:{limit:17,page:1,smtTime:rq_time},
