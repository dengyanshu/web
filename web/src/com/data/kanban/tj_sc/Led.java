...
public class Led extends Action{
	...
	public void getResult(){
		...
		try {
			...
			getCs().setString("time", request.getParameter("LedTime"));
			getCs().execute(); 
	...
	public void getResult2(){
		setConn(new TjDatabase().getConn());
		try {
			setCs(getConn().prepareCall("{call NewLedSc(?,?,?)}"));
