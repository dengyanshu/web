...
public class Xghsyffsm extends ActionSupport{
	private String key;
	private String sn;
	...
	public String execute()throws Exception{
		...
		try{
			...
			if(this.key.equals("output")){			
				/** 
				 * 锡膏发放扫描程序
				 **/
				stmt=conn.prepareCall("{call Txn_TinolPutOut(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
				//系统平台默认的参数：
				...
				this.success=true;
			}else if(this.key.equals("input")){			
				/** 
				 * 锡膏回收扫描程序
				 **/
				stmt=conn.prepareCall("{call Txn_TinolHW(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
				//系统平台默认的参数：
				stmt.setString("I_Sender", "");
				stmt.registerOutParameter("I_ReturnMessage", Types.VARCHAR);
				stmt.registerOutParameter("I_ExceptionFieldName", Types.VARCHAR);
